...
if __name__ == '__main__':
    ...
    # Run evaluation for 1D and 2D MDS

    print('\nRun evaluation comparing to MDS 2D')
    mds_2d = MDS(n_components=2, random_state=777)
    run_evaluation(sample_data[['x', 'y', 'z']], sample_data['target'], res,
                   mds_2d.fit_transform(sample_data[['x', 'y', 'z']]))

    print('\nRun evaluation comparing to MDS 1D')
    mds_1d = MDS(n_components=1, random_state=777)
    run_evaluation(sample_data[['x', 'y', 'z']], sample_data['target'], res,
                   mds_1d.fit_transform(sample_data[['x', 'y', 'z']]))
