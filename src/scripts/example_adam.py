...
from src.projections.circular_projection_adam import circular_projection_adam  # Import the Adam version
...
def plot_circular_projection(df, res):
    ...
    fig, ax = plt.subplots()
    ax.set_aspect('equal')
    plt.title('Circular Projection of the Data Points (Adam)')
    plt.xlim(-1.1, 1.1)
    plt.ylim(-1.1, 1.1)
    plt.xlabel("x")
    plt.ylabel("y")
...
def plot_loss(res):
    ...
    plt.ylabel('Loss')
    plt.title('Adam Optimization: Loss Reduction Over Iterations')
...
def plot_distances(res):
    ...
    plt.title('High-dimensional vs. Low-dimensional Distances (Adam)')
...
if __name__ == '__main__':
    ...
    # Convert the DataFrame to a NumPy array before passing to the circular projection function
    data_as_array = sample_data[['x', 'y', 'z']].to_numpy()

    # Call the Adam-based circular projection
    res = circular_projection_adam(data_as_array)
